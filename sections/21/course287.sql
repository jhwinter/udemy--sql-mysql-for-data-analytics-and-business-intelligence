...
WITH cte AS (SELECT AVG(salary) AS avg_salary FROM salaries)
SELECT 
	SUM(CASE WHEN s.salary <= c.avg_salary THEN 1 ELSE 0 END) num_of_salaries_below_avg,
...
WITH cte AS (SELECT AVG(salary) AS avg_salary FROM salaries)
SELECT 
	COUNT(CASE WHEN s.salary <= c.avg_salary THEN s.salary ELSE NULL END) num_of_salaries_below_avg,
	COUNT(s.salary) AS num_of_salary_contracts
FROM salaries s
	JOIN employees e ON s.emp_no = e.emp_no AND e.gender = 'M'
    CROSS JOIN cte c;
...
SELECT 
	SUM(CASE WHEN s.salary <= c.avg_salary THEN 1 ELSE 0 END) num_of_salaries_below_avg,
	COUNT(s.salary) AS num_of_salary_contracts
FROM (SELECT AVG(salary) AS avg_salary FROM salaries) c
	JOIN salaries s
	JOIN employees e ON s.emp_no = e.emp_no AND e.gender = 'M';
